...
6. Реализуем сервис-адаптер, который будет получать изменения в таблице БД монолита через Kafka и отправлять соответствующие запросы в DeviceService и LocationService. ЯП: Python

Примечания и допущения: 
1. Учитывая, что в текущем примере монолита нет привязки устройства к пользователю, то в новой реализации будем считать, что все устройства автоматически привязываются к пользователю с user_id = 1, а дальше их вручную перепривязывают специалисты.
