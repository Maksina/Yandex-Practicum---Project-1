...
Таким образом переходная схема компонентов:

В данном задании:
1. Настроим и запустим Kafka с топиком "smarthome-db.public.sensors", в котором записываются все изменения таблицы БД Sensors из монолита.
2. Настроим и запустим CDC(Debezium), который подключается к БД Монолита и реплицирует данные в Kafka "smarthome-db.public.sensors"
3. Реализуем DeviceService для управления устройствами. ЯП: Java
4. Реализуем LocationService для управления помещениями. ЯП: Java
5. Реализуем TelemetryService для управления телеметрией. ЯП: Java
5. Реализуем сервис-адаптер, который будет получать изменения в таблице БД монолита через Kafka и отправлять соответствующие запросы в DeviceService и LocationService. ЯП: Python

Примечания и допущения: 
1. Учитывая, что в текущем примере монолита нет привязки устройства к пользователю, то в новой реализации будем считать, что все устройства автоматически привязываются к пользователю с user_id = null, а дальше их вручную перепривязывают специалисты.
2. Учитывая текущую реализацию монолита, интеграция с ним сервиса телеметрии лишена смысла. Поэтому сервис телеметрии будет реализовывать последним, как завершающий этап перевода монолита на микросервисы.
