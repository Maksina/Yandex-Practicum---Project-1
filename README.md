...
2. Обеспечиваем миграцию данных из БД Монолита в БД микросервисов:
	- Подключаем CDC (Debezium), который реплицирует все изменения БД и публикует их в Kafka
	- Реализовываем и подключаем сервис-адаптер, который подписывается на соответствующий топик Kafka и вызывает реализованные микросервисы
